...
const FilterAccordion: React.FC<FilterProps & CalendarProps> = React.forwardRef(
  ({ updateFilters, currentFilters, updateCalendarOpened }, _) => {
    const [opened, setOpened] = useState<string[]>([]);

    const [calendarOpened, setCalendarOpened] = useState<boolean>(false);

    const [femaleFilterChecked, setFemaleFilterChecked] = useState<boolean>(
      currentFilters.gender === "Female"
    );
    const [maleFilterChecked, setMaleFilterChecked] = useState<boolean>(
    ...
    const handleStartDateChange = (newStartDate: Date | null) => {
      const startDateToUse =
        newStartDate || new Date("0000-01-01T00:00:00.000Z");
    ...
    const genderFilter = (
      <Group justify="space-between" gap="xl" grow>
        <Checkbox
          ...
          onChange={(event) =>
            handleCheckmarkChange(event.currentTarget.checked, "Female")
          }
        />
        <Checkbox
          ...
          onChange={(event) =>
            handleCheckmarkChange(event.currentTarget.checked, "Male")
          }
        />
      </Group>
    );
    ...
    const textAgeControl = (
      <Group>
        <TextInput
          ...
          onKeyDown={(event) => {
            if (event.key === "Enter")
              changeMinAgeFromText(event.currentTarget.value);
          }}
        />
        <TextInput
          mb={minAgeError ? "md" : ""}
          ...
          onKeyDown={(event) => {
            if (event.key === "Enter")
              changeMaxAgeFromText(event.currentTarget.value);
          }}
        />
      </Group>
    );
